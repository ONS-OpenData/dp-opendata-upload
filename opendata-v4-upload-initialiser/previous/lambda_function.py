...
def lambda_handler(event, context):
    ...
    assert (
        len(records) == 1
    ), f"This lambda should be triggered be receiving exactly one record of a file in the bucket being updated. Got {len(records)}"
    record = records[0]
    ...
    try:
        ...
        raise KeyError(
            f'Could not find ["s3"]["bucket"]["name"] in Records[0] of event {json.dumps(event, indent=2)}'
        )
    ...
    try:
        ...
        raise KeyError(
            f'Could not find ["s3"]["object"]["key"] in Records[0] of event {json.dumps(event, indent=2)}'
        )

    # get metadata from s3 object
    object_response = s3.head_object(Bucket=bucket, Key=v4_file)
    try:
        metadata = object_response["Metadata"]["dataset-details"].replace(
            "'", '"'
        )  # json.loads only picks ups double quotes
    ...
    assert (
        dataset_id
    ), f'could not get required "dataset_id" key from metadata_dict dict {json.dumps(metadata_dict, indent=2)}'
